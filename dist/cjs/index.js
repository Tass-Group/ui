...
const renderMenuItem = (item) => {
    if ((item.children?.length) != null) {
        return (jsxRuntime.jsxs(SubMenu, { children: [jsxRuntime.jsxs(MenuItem, { children: [(item.icon != null) && jsxRuntime.jsx("span", { className: "menu-item-icon", children: item.icon }), item.label, jsxRuntime.jsx(Icon, { name: "chevronright", size: 12, style: { marginLeft: "auto" } })] }), jsxRuntime.jsx(SubMenuContent, { className: "submenu-content", children: item.children.map(child => renderMenuItem(child)) })] }, item.key));
    }
    return (jsxRuntime.jsxs(MenuItem, { disabled: item.disabled, isdanger: item.isdanger, onClick: item.onClick, children: [(item.icon != null) && jsxRuntime.jsx("span", { className: "menu-item-icon", children: item.icon }), item.label] }, item.key));
};
const Dropdown = ({ items, trigger = "hover", placement = "bottom", children, className, style }) => {
    ...
    return (jsxRuntime.jsxs(DropdownContainer, { ref: containerRef, className: className, style: style, onClick: handleTrigger, onMouseEnter: handleMouseEnter, onMouseLeave: handleMouseLeave, children: [children, visible && (jsxRuntime.jsx(MenuContainer, { placement: placement, children: items.map(item => renderMenuItem(item)) }))] }));
};
