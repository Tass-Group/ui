...
import React__default, { useDebugValue, createElement, useRef, useContext, useState, useEffect } from 'react';
import 'react-dom';
...
const Button = ({ title = "Button", type = "default", size = "medium", shape = "square", disabled = false, icon, ...rest }) => {
    return (jsxs(StyledButton, { type: type, size: size, shape: shape, disabled: disabled, ...rest, children: [icon && jsx("span", { className: "button-icon", children: icon }), title] }));
};
...
const Title = ({ level = 1, children, className, style, ellipsis, width }) => {
    const Tag = `h${level}`;
    return jsx(Tag, { className: className, style: { ...style, ...getEllipsisStyle(ellipsis, width) }, children: children });
};
const Text = ({ children, className, style, strong, italic, underline, delete: del, code, mark, keyboard, ellipsis, width }) => {
    let textElement = jsx(Fragment, { children: children });
    if (strong === true) {
        textElement = jsx("strong", { children: textElement });
    }
    if (italic === true) {
        textElement = jsx("em", { children: textElement });
    }
    if (underline === true) {
        textElement = jsx("u", { children: textElement });
    }
    if (del === true) {
        textElement = jsx("del", { children: textElement });
    }
    if (code === true) {
        textElement = jsx("code", { children: textElement });
    }
    if (mark === true) {
        textElement = jsx("mark", { children: textElement });
    }
    if (keyboard === true) {
        textElement = jsx("kbd", { children: textElement });
    }
    return jsx("div", { className: className, style: { ...style, ...getEllipsisStyle(ellipsis, width) }, children: textElement });
};
const Paragraph = ({ children, className, style, blockquote, ellipsis, width }) => {
    const paragraphElement = jsx("p", { className: className, style: { ...style, ...getEllipsisStyle(ellipsis, width) }, children: children });
    return (blockquote === true) ? jsx("blockquote", { "data-testid": "blockquote", children: paragraphElement }) : paragraphElement;
};
const Typography = ({ children, className, style, onClick }) => {
    return (jsx("div", { className: className, style: style, onClick: onClick, children: children }));
};
...
const Icon = ({ name, size = 16, color = "currentColor", className = "", style = {}, onClick }) => {
    return (jsx("span", { className: `icons icon-${name} ${className}`, style: {
            fontSize: size,
            color,
            ...style
...
const Divider = ({ className = "", orientation = "horizontal", color, text, position = "center", style = {}, variant = "solid" }) => {
    return (jsx(StyledDivider, { role: "separator", className: className, orientation: orientation, color: color, position: position, style: style, variant: variant, children: (text != null)
            ? (jsxs(Fragment, { children: [position !== "right" && jsx("span", { className: "line" }), jsx("span", { className: "text", children: text }), position !== "left" && jsx("span", { className: "line" })] }))
            : (jsx("span", { className: "line", style: { margin: 0 } })) }));
};
...
const Row = ({ children, gutter = 0, ...rest }) => {
    return (jsx(StyledRow, { gutter: gutter, ...rest, children: React__default.Children.map(children, (child) => {
            if (React__default.isValidElement(child)) {
                return React__default.cloneElement(child, { gutter });
            }
            return child;
        }) }));
};
...
const Col = ({ children, span, gutter = 0, offset = 0, ...rest }) => {
    return (jsx(StyledCol, { span: span, gutter: gutter, offset: offset, ...rest, children: children }));
};
...
const Space = ({ children, size = 8, direction = "horizontal" }) => {
    return jsx(SpaceContainer, { size: size, direction: direction, children: children });
};
...
const Breadcrumb = ({ items = [], separator = "/" }) => {
    return (jsx(BreadcrumbContainer, { children: items.map((item, index) => {
            const isLast = index === items.length - 1;
            return (jsxs(React__default.Fragment, { children: [jsx(BreadcrumbItem, { children: (item.path != null) && !isLast
                            ? (jsx(Link, { to: item.path, children: item.title }))
                            : (item.title) }), !isLast && jsx(Separator, { children: separator })] }, index));
        }) }));
};
...
const Dropdown = ({ items, trigger = "hover", placement = "bottom", children, className, style }) => {
    ...
    return (jsxs(DropdownContainer$1, { ref: containerRef, className: className, style: style, onClick: handleTrigger, onMouseEnter: handleMouseEnter, onMouseLeave: handleMouseLeave, children: [children, visible && (jsx(MenuContainer$1, { placement: placement, children: items.map(item => renderMenuItem(item)) }))] }));
};
...
const Cascader = ({ options = [], defaultValue, value: propValue, onChange, placeholder = "Please select", disabled = false, allowClear = true, expandTrigger = "click", displayRender = labels => labels.join(" / "), className, style }) => {
    ...
    return (jsxs(CascaderContainer, { ref: containerRef, className: className, style: style, children: [jsxs(CascaderInput, { role: "combobox", "aria-expanded": visible, "aria-haspopup": "listbox", onClick: () => { !disabled && setVisible(!visible); }, disabled: disabled, children: [jsx("span", { className: "label", children: (value.length > 0) ? displayLabel : placeholder }), allowClear && value.length > 0 && !disabled && (jsx(ClearIcon, { onClick: handleClear, children: jsx(Icon, { name: "cross", size: 12 }) })), jsx(ArrowIcon, { "$visible": visible, children: jsx(Icon, { name: "chevrondown", size: 12 }) })] }), visible && (jsx(DropdownContainer, { children: activeOptions.map((options, level) => (jsx(OptionList, { children: options.map(option => (jsxs(OptionItem, { disabled: option.disabled, selected: value[level] === option.value, onMouseEnter: () => {
